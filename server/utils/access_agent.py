...
class Endpoint(AgentBase):
    ...
    def __init__(self, kiosk_id, log, default_host, default_port, timeout, side="server"):
        ...
        if side.lower() not in ("server", "kiosk"):
            raise UnknownException("Invalid Endpoint side %s" % side)
        self.side = side
    ...
    def get_host_port(self):
        ...
        return self.default_host, int(self.default_port)
        port = ""
...
class Server(Endpoint):
    ...
    def __call__(self, func_name, params):
        ...
        try:
            ...
            self.log.exception("Error when execute %s(%s): %s" % (func_name, params, ex))
            raise
